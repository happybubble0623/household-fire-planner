...
- `expectedAnnualPortfolioReturnPercent` (Portfolio Drawdown: total return, UI label `Expected total return`. Principal-Preserving: appreciation only, UI label `Asset appreciation (not spent)`. Unused in Income Stream.)
- `expectedCashGeneratingReturnPercent` (Principal-Preserving only: spendable cash yield, UI label `Spendable investment return`.)
- `inflationRatePercent`
- `withdrawalRatePercent`
- `taxMode`
- `simpleEffectiveTaxRatePercent`
- `homeSaleAge` and `homeSaleProceeds` (one-time home/real-estate sale added to liquid FIRE assets at the sale age; default 0/0 = no sale. Applies to Portfolio Drawdown and Principal-Preserving; ignored by Income Stream.)
...
- Simple tax mode (Option B) is available in all three modes: it grosses up the full spending need by `expenses / (1 - rate)` so every funding source is pre-tax. Real estate is excluded from the drawdown pool (liquid assets only).
- Principal-Preserving applies appreciation to grow the unspent principal after FIRE; income + cash yield fund spending; the mode passes only while assets stay at or above the FIRE-age principal floor.
...
- Do not break `phase1.7` migration defaults.
...
- Principal-Preserving FIRE includes cash-generating return.
