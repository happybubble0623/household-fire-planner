...
// Server-renderable Path-to-FIRE hub. This is the single source of the hub
// markup, shared by:
//   • the public homepage "/" (rendered as a Server Component → real HTML in the
//     initial response for SEO + instant first paint), and
//   • the in-app /app/fire-path workspace (via PathToFirePanel), which the iOS
//     app loads.
// It uses no client hooks itself — `isAppMode` and `status` arrive as props — so
// it renders on the server for the homepage while still working inside the
// client workspace. The only interactive island is <PathPicker /> (a client
// component), exactly as the original design intended.
  ...
  return (
    <div className="aurora-home">
      ...
      <div className="wrap">
          ...
          <div className={isAppMode ? "paths-grid" : "paths-grid four"}>
            {(isAppMode
              ...
              >
                ...
                <Link href={card.href} className="paths-btn">
                  Start this path →
                </Link>
              </div>
            ))}
        ...
        <div className="foot">
          <span>© 2026 Plan My FIRE · Planning estimates only. Not financial advice.</span>
          ...
          {status ? <span>Local workbook: {status}</span> : null}
        </div>
      </div>
    </div>
  );
