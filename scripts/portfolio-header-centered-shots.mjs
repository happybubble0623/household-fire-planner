// One-off: capture the Portfolio header card after removing the duplicate
// "Local mode. Saved on this device." status line (the left-side
// "Saved locally, sync optional" chip remains) and vertically centering the
// "Update today's prices" / "Showing EOD prices for <date>" cluster.
//
// Seeds a small workbook (a few holdings + an EOD refresh date) straight into
// the app's Dexie/IndexedDB store so the populated state renders, then reloads.
// Run against the local dev server: node scripts/portfolio-header-centered-shots.mjs
...
// The header card: no "Local mode…" line, controls vertically centered.
const headerCard = page
  .locator("h1", { hasText: "household portfolio" })
  .locator("xpath=ancestor::div[contains(@class,'rounded-2xl')][1]");
await headerCard.screenshot({ path: join(out, "portfolio-header-centered-controls.png") });
console.log("saved portfolio-header-centered-controls.png");
