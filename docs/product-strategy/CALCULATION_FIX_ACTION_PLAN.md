...
> *Original status (pre-fix):* All seven were reproduced by running the real code with
> hard expected-vs-actual figures, and each had a queued `it.skip` regression test waiting
...
Fix **#7** (investment fee input) shipped with a **new** `INV-fee` rule-pinned test
